...
layui.use(['util', 'layer', 'table'], function () {
    $(document).ready(function () {
            ...
            if (layEvent === 'detail') { //查看功能
                ...
                layer.prompt({title: '请输入楼层名称', formType: 0, value: floorName, offset: '220px', maxlength: 10
                    ...
                    $.post(baseUrl + '/QueryFloorNameServlet', NewfloorName, function updateCheck(data) {
                        if (data === "1") {
                            layer.close(index);
                            table.reload('tableID', {where: {make: 2, floorId: floorId, floorName: value}});
                            layer.msg('修改结果如下', {offset: '250px'});
                        } else {
                            layer.alert('该楼层名称已经存在！', {title: '警告', icon: 4, anim: 6, offset: '220px'});
                        }
                    });
                });
            }
        ...
        $('#searchButton').click(function () {
            ...
            if (inputTxt === "")
                ...
                table.reload('tableID', {
                    where: {make: 3, floorName: inputTxt}
                })
            }
        });
        ...
        $('#insertButton').click(function () {
            layer.prompt({title: '请输入楼层名称', formType: 0, offset: '220px', maxlength: 10
                ...
                $.post(baseUrl + '/QueryFloorNameServlet', params, function insertCheck(data) {
                    if (data === "1") {
                        layer.close(index);
                        table.reload('tableID', {where: {make: 1, floorName: inputValue}});
        ...
        $('#toXlsButton').click(function () {
            ...
            layer.prompt({title: '请输入文件名称', formType: 0, value: 'FloorInfo', offset: '220px'
