...
layui.use(['util', 'layer', 'table'], function () {
    $(document).ready(function () {
        ...
        table.on('tool', function (obj) { //tool是工具条事件名
            ...
            if (layEvent === 'detail') { //查看功能
                layer.alert('权限项：' + data.authItem + '<br>最低可读权限等级：' + data.isRead + '<br>最低可写权限等级：'
                    ...
                    , {skin: 'layui-layer-lan', closeBtn: 0, title: '您当前选择的权限值信息', anim: 4, offset: '180px'});

            } else if (layEvent === 'del') { //删除功能
                layer.alert('本条目禁止删除！', {title: '警告', icon: 4, anim: 6, offset: '250px'});

            } else if (layEvent === 'edit') { //编辑功能
                //layer.prompt(options, yes) - 输入层
                //formType:输入框类型，支持0（文本）默认1（密码）2（多行文本） maxlength: 140, //可输入文本的最大长度，默认500
                layer.prompt({
                    title: '请输入最低可读权限等级', formType: 0, value: isRead, offset: '220px', maxlength: 1
                }, function (IsRead, index) {
                    layer.close(index);
                    layer.prompt({
                        title: '请输入最低可写权限等级', formType: 0, value: isWrite, offset: '220px', maxlength: 1
                    }, function (IsWrite, index) {
                        layer.close(index);
                        layer.prompt({
                            title: '请输入最低可改权限等级', formType: 0, value: isChange, offset: '220px', maxlength: 1
                        }, function (IsChange, index) {
                            layer.close(index);
                            layer.prompt({
                                title: '请输入最低可改权限等级', formType: 0, value: isDelete, offset: '220px', maxlength: 1
        ...
        $('#insertAuth').click(function () {
            layer.alert('本条目禁止新增！', {title: '警告', icon: 4, anim: 6, offset: '250px'});
        });
        ...
        $('#searchAuthITEM').click(function () {
            ...
            if (authItem === "")
                ...
                table.reload('tableAuth', {where: {make: 3, authItem: authItem}});
            }
